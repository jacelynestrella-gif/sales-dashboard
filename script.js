...
function updateActualVsTarget(rows) {
  ...
  actualTargetChart = new Chart(
    document.getElementById("actualTargetChart"),
    {
      type: "bar",
...
function updateBrandTargetTable(rows) {
  ...
  document.getElementById("brandTargetTable").innerHTML =
    html +
    `
      <tr class="total-row">
        <td>TOTAL</td>
...
function updateDealers(rows) {
  ...
  const topDealers = rows
    .sort((a, b) =>
      cleanNumber(b["Total Sales"]) -
      cleanNumber(a["Total Sales"])
    )
    .slice(0, 10);
